...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [eventsRes, bookingsRes] = await Promise.all([
        api.get("/events"),
        api.get("/booking/all"), // রাউটটি চেক করে নেবেন
      ]);
      ...
      // 💡 যদি ব্যাকএন্ড ডাটা অবজেক্টের ভেতর পাঠায় (যেমন res.data.bookings) তবে সেটা ধরবে, নাহলে সরাসরি ডাটাক ধরবে
  ...
  const handleCancelBooking = async (id) => {
    if (window.confirm("Cancel this user's booking request?")) {
      try {
        await api.delete(`/booking/${id}/admin`);
        fetchData();
      } catch (error) {
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <h4 className="font-bold text-gray-900 text-lg leading-tight">
                {/* 🔄 ফিক্সড: booking.eventId এর জায়গায় booking.event হবে */}
            ...
            <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100 text-sm">
              ...
              <p className="text-gray-700 flex items-center gap-2 mb-1">
                ...
                <span>
                  {/* 📝 নোটিশ: এটি বুকিং করার সময় (bookedAt)। আপনি চাইলে ইভেন্টের ডেটও দেখাতে পারেন booking.event?.date দিয়ে */}
                  {booking.createdAt ? new Date(booking.createdAt).toLocaleString() : "Invalid Date"}
                </span>
              </p>
              {/* 🔄 ফিক্সড: booking.eventId এর জায়গায় booking.event হবে */}
              {booking.event && (
                <p className="text-gray-700 flex items-center gap-2 mt-2 pt-2 border-t border-gray-200">
                  ...
                  <span
                    className={`font-bold ${booking.event.availableSeats > 0 ? "text-green-600" : "text-red-500"}`}
                  >
                    {booking.event.availableSeats}
                  </span>{" "}
                  remaining of {booking.event.totalSeats}
                </p>
              )}
            </div>
