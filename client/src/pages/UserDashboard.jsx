                    ...
                    {bookings.map((booking) => (
                        <div key={booking._id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition border border-gray-100 flex flex-col">
                            <div className="p-6 border-b border-gray-50 flex-grow">
                                {/* ✅ এখানে eventId বদলে event করা হয়েছে */}
                                {booking.event ? (
                                    <>
                                        <div className="flex justify-between items-start mb-4">
                                            <h3 className="text-lg font-bold text-gray-900 leading-tight">{booking.event.title}</h3>
                                        ...
                                        <div className="text-sm text-gray-500 mb-4 space-y-1">
                                            <p><strong className="text-gray-700">Date:</strong> {booking.event.date ? new Date(booking.event.date).toLocaleDateString() : 'N/A'}</p>
                                            <p><strong className="text-gray-700">Amount:</strong> {booking.amount === 0 ? 'Free' : `₹${booking.amount}`}</p>
                                            <p><strong className="text-gray-700">Requested:</strong> {booking.bookedAt ? new Date(booking.bookedAt).toLocaleDateString() : new Date().toLocaleDateString()}</p>
                                        </div>
                                    </>
                                ) : (
                            ...
                            <div className="p-4 bg-gray-50 flex justify-between items-center shrink-0">
                                {booking.event && booking.status !== 'cancelled' ? (
                                    <>
                                        <Link to={`/events/${booking.event._id}`} className="text-gray-900 font-semibold text-sm hover:underline">View Event</Link>
                                        <button
